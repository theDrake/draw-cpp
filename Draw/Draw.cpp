...
void display(void)
{
	...
	for (buttonIter = gButtons.begin(); buttonIter < gButtons.end(); ++buttonIter)
	{
		(*buttonIter)->Draw();
	}
...
void mouse(int mouse_button, int state, int x, int y)
{
	...
	if (mouse_button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) 
	{
		...
		{
			// If not already dragging, but a point has been clicked, select it for dragging:
			if (!gLeftDragging)
			{
				vector<Point2D *>::iterator pointIter;
				for (pointIter = gPoints.begin(); pointIter < gPoints.end(); ++pointIter)
			...
			if (!gLeftDragging)
			{
				...
				case RECTANGLE:
					...
					{
						DeselectAllShapes();
						gShapes.push_back(new Rectangle(gPoints, gRed, gGreen, gBlue, gFilled));
						gPoints.clear();
					}
					break;
				case TRIANGLE:
					...
					{
						DeselectAllShapes();
						gShapes.push_back(new Triangle(gPoints, gRed, gGreen, gBlue, gFilled));
						gPoints.clear();
					}
					break;
				case PENTAGON:
					...
					{
						DeselectAllShapes();
						gShapes.push_back(new Pentagon(gPoints, gRed, gGreen, gBlue, gFilled));
						gPoints.clear();
					}
					break;
				case CIRCLE:
					...
					{
						DeselectAllShapes();
						gShapes.push_back(new Circle(gPoints, gRed, gGreen, gBlue, gFilled));
						gPoints.clear();
					}
					break;
				default:
		...
		else
		{
			...
			for (buttonIter = gButtons.begin(); buttonIter < gButtons.end(); ++buttonIter)
			{
				if ((*buttonIter)->Contains(x, y))
				{
					...
					{
						((Slider *) *buttonIter)->SetSliderLength(x - (*buttonIter)->GetLeft());

						double newRGB = (x - (*buttonIter)->GetLeft()) / (*buttonIter)->GetLength();
					...
					else if ((*buttonIter)->IsButtonType(LOAD_BUTTON))
					{
						...
						while (fin.good())
						{
							...
							if (fin.good())
							{
								...
								for (doubleIter = input.begin(); distance(doubleIter, input.end()) > 4; doubleIter += 2)
								{
									gPoints.push_back(new Point2D(*doubleIter, *(doubleIter + 1)));
								}
	...
	// Whenever the left button is released, ensure no point is selected for dragging:
	...
	// Whenever the right button is released, ensure no point is selected for dragging:
...
void InitializeMyStuff()
{
	...
	AddLabel(DEFAULT_BUTTON_MARGIN_X,
			 gScreenY - n * DEFAULT_BUTTON_MARGIN_Y - (n - 1) * DEFAULT_BUTTON_HEIGHT,
			 DEFAULT_BUTTON_MARGIN_X + DEFAULT_BUTTON_WIDTH,
			 gScreenY - n * DEFAULT_BUTTON_MARGIN_Y - n * DEFAULT_BUTTON_HEIGHT,
			 CONTROL_PANEL_RED,
			 CONTROL_PANEL_GREEN,
			 CONTROL_PANEL_BLUE,
			 "Drawing Mode:");
